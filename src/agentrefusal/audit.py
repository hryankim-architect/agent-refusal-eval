"""Append-only chained NDJSON ledger for screening decisions (mirrors the substrate).

Every gate decision — especially a refusal — writes an entry here, so the
screening itself is tamper-evident: editing any earlier entry breaks the chain.
That auditability is the regulatory-grade property the eval checks (see
``scorer.audit_completeness``).
"""
